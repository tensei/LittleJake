...
namespace LittleSteve.Modules
{
    ...
    public class YoutubeModule : ModuleBase<SteveBotCommandContext>
    {
        ...
        public async Task AddTwitch(string youtubeId, IGuildChannel guildChannel)
        {
            ...
            var youtuber = await _botContext.Youtubers.Include(x => x.YoutubeAlertSubscriptions).FirstOrDefaultAsync(x => x.Id == youtubeId);
            ...
            if (youtuber is null)
            {
                ...
                JobManager.AddJob(() => new YoutubeMonitoringJob(youtuber.Id,Context.Provider.GetService<SteveBotContext>(), Context.Client).Execute(), s => s.WithName(youtubeId).ToRunEvery(60).Seconds());
            }
